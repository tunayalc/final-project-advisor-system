...
function calculateQuotas() {
    const db = getDb();
    const activeFaculty = db.prepare('SELECT id, department_id, current_quota FROM faculty WHERE is_active = 1 ORDER BY department_id ASC, id ASC').all();
    const inactiveFaculty = db.prepare('SELECT id, current_quota FROM faculty WHERE is_active = 0').all();
    const unassignedByDepartment = db.prepare(`
...
function runAssignment() {
    ...
    const transaction = db.transaction(() => {
        ...
        const facultyList = db.prepare('SELECT id, department_id, base_quota, current_quota FROM faculty WHERE is_active = 1').all();
        ...
        for (const candidate of scoredCandidates) {
            ...
            updateStudent.run(candidate.faculty_id, candidate.student_id);
