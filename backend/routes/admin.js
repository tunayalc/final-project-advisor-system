...
router.post('/force-assign', authenticate, authorize('admin'), (req, res) => {
    try {
        ...
        const db = getDb();
        const targetFaculty = db.prepare('SELECT id, is_active, department_id FROM faculty WHERE id = ?').get(faculty_id);
        ...
        db.transaction(() => {
            ...
            const student = db.prepare('SELECT assigned_faculty_id, department_id, approval_status FROM students WHERE id = ?').get(student_id);
            if (!student) {
...
router.get('/student-applications', authenticate, authorize('admin'), (req, res) => {
    try {
        const db = getDb();
...
router.patch('/students/:studentId/review', authenticate, authorize('admin'), (req, res) => {
    try {
        ...
        const db = getDb();
        const student = db.prepare('SELECT id, user_id, assigned_faculty_id FROM students WHERE id = ?').get(studentId);
        if (!student) {
        ...
        db.transaction(() => {
            ...
            if (normalizedStatus !== 'approved') {
                ...
                db.prepare("UPDATE pre_assignments SET status = 'rejected' WHERE student_id = ? AND status = 'pending'").run(studentId);
            }
